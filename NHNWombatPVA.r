...
LK.pr.breed <- 0.6666
LK.breed.f <- LK.pr.breed/5 # females. sex ratio 1:1 historically
...
write.csv(alldata, file = "LK40GalldataQ.csv")
...
yr.now <- 2020 # 
yr.end <- yr.now + round(100,0) # set projection end date to 40 generations (IUCN)
t <- (yr.end - yr.now) #time frame
yr.vec <- seq(yr.now,yr.end) # year vector
...
alldata <- data.frame(pop.found.vec, minNmd, minNlo, minNup, PrQExt)
write.csv(alldata, file = "LK100Yalldata.csv")
