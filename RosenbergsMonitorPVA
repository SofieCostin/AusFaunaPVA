...
# Rosenbergs Goanna PVA
# requires library - Plotly
# update 23/06/2022

# Rismiller et al 2010
...
age.max = 30 # maximum age of females in wild from heath goanna fact sheet
...
prob.breed.all <- c(4/7, 4/13, 4/6, 3/5, 3/6, 9/15, 5/7, 4/7, 4/7, 3/4)
prob.breed <-sum(prob.breed.all/10)
no.eggs.f <- (prob.breed*14)/2
#no fertile eggs in final two years of life


m.vec <- c((rep(0, 10)), (rep(no.eggs.f, age.max-11)), 0, 0) ## female offspring produced each year
plot(0:age.max+1, m.vec,pch=19,type="b")

# fertility errors
m.sd.vec <- c((rep(0, 10)), (rep(0.2, age.max-11)), 0, 0) #mean and standard deviations vector, juvenile and adult fertility 

# Create vectors: Survival ------------------------------------------------

#survival
s.vec <- c(0.08, rep(0.95, age.max-1), 0)  ## female  survival # probability of surviving from one year to the next. e.g surviving fourth year of life
...
red.vec <- c(1,0.99999,0.9985,0.995,0.985)
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      if (rbinom(1, 1, 0.14/gen.l) == 1) { # catastrophe
        ...
        s.stoch1 <- s.stoch * (rbeta(1, cat.alpha, cat.beta))
        s.stoch <- ifelse(is.na(s.stoch1)==T, 0, s.stoch1)
      }
