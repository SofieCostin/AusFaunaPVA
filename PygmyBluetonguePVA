...
# Phgmy Bluetongue  PVA
# requires library - Plotly
# update 21/06/2022

# All demographic values from Milne 1999, Conservation and Ecology of the Endangered Pigmy Bluetongue Lizard "Tiliqua adelaidensis" PhD thesis
...
age.max <- 9 # known max age of PBT in the wild 

age.vec <- 0:age.max

# Create vectors: fertility -----------------------------------------------

# 81.1% of females predicted to breed each year
# average litter size 3.23 +/- 0.75 young
# fecundity values from pg206


m.vec <- c(0,0,(0.896), (1.19), (1.33), rep(1.62, age.max-4)) # female offspring produced per year
m.vec


# fertility errors

m.sd.vec <- c(rep(0.2, age.max+1)) #mean and standard deviations vector, juvenile and adult fertility 
m.sd.vec

# Create vectors: Survival ------------------------------------------------

s.vec <- c(1.0, 0.253, rep(0.767, age.max-1)) # corrected for migration
s.vec ## female  survival # probability of surviving from one year to the next. e.g surviving fourth year of life

# survival errors

s.sd.vec <- c(rep(0.13, age.max+1)) #mean and standard deviations vector, juvenile and adult survival
s.sd.vec
...
popmat[age.max+1,age.max+1] <- s.vec[age.max+1] #  fill age max, age max with s.vec
...
red.vec <- c(1,0.995,0.984,0.978,0.968)
...
plot(pop.found.vec, minNmd, type="l", xlab="founding N", ylab="minimum N")
